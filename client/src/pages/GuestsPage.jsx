...
export default function GuestsPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', phone: '', dietaryPreference: 'None', rsvpStatus: 'PENDING', plusOne: false,
  });
  ...
  const load = () => {
    ...
    Promise.all([
      guestApi.getGuests(eventId, params),
      guestApi.getStats(eventId),
      guestApi.getFeedback(eventId),
    ]).then(([gRes, sRes, fRes]) => {
  ...
  return (
    <div>
      ...
      <div className="filters-bar">
        ...
        <div className="form-group">
          <label>Search</label>
          <input className="form-control" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Guest name..." />
        </div>
      ...
      {loading ? <Loading /> : (
        <div className="card">
          <table>
            <thead>
              <tr><th>Name</th><th>Email</th><th>RSVP</th><th>Dietary</th><th>Plus One</th><th>Invited</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {guests.map((g) => (
                <tr key={g.id}>
                  ...
                  <td>{g.dietaryPreference || 'None'}</td>
                  <td>{g.plusOne ? 'Yes' : 'No'}</td>
                  <td>{g.invitationSentAt ? '✓' : '—'}</td>
      ...
      {stats?.dietary && Object.keys(stats.dietary).length > 0 && (
        <div className="card">
          <div className="card-title">Dietary Preferences Breakdown</div>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {Object.entries(stats.dietary).map(([diet, count]) => (
              <span key={diet} className="badge badge-gray">{diet}: {count}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add Guest' : 'Edit Guest'} onClose={() => setModal(null)} onSubmit={save}>
          ...
          <div className="form-group">
            <label><input type="checkbox" checked={form.plusOne} onChange={(e) => setForm({ ...form, plusOne: e.target.checked })} /> Plus One</label>
          </div>
        </Modal>
      )}
    </div>
