...
import { stakeholderApi } from '../../api/index.js';
...
export default function StakeholdersPage() {
  ...
  const load = () => { stakeholderApi.getAll(eventId).then((res) => setItems(res.data.data)).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, [eventId]);

  const openAdd = () => { setForm({ name: '', email: '', phone: '', role: 'OTHER', notes: '' }); setModal('add'); };
  const openEdit = (s) => { setForm(s); setModal(s.id); };
  const save = async () => {
    if (modal === 'add') await stakeholderApi.create(eventId, form);
    else await stakeholderApi.update(eventId, modal, form);
    setModal(null); load();
  };
  const remove = async (id) => {
    if (!confirm('Remove stakeholder?')) return;
    await stakeholderApi.remove(eventId, id); load();
  };

  return (
    <div>
      <EventSubNav />
      <div className="page-header"><h1>Stakeholders</h1><button className="btn btn-primary" onClick={openAdd}>+ Add Stakeholder</button></div>
      {loading ? <Loading /> : (
        <div className="card">
          <table>
