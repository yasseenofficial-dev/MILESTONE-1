...
import { staffApi, taskApi } from '../../api/index.js';
...
export default function StaffPage() {
  ...
  const load = () => {
    ...
    Promise.all([staffApi.getAll(eventId), taskApi.getTasks(eventId)])
      .then(([sRes, tRes]) => { setStaff(sRes.data.data); setTasks(tRes.data.data.filter((t) => !t.staffId)); })
    ...
    <div>
              ...
              {tasks.length > 0 && (
                <div style={{ marginTop: '0.75rem' }}>
                  <select className="form-control" style={{ fontSize: '0.8rem' }}
                    onChange={(e) => { if (e.target.value) { staffApi.assignTask(eventId, s.id, e.target.value).then(load); e.target.value = ''; } }}>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add Staff' : 'Edit Staff'} onClose={() => setModal(null)} onSubmit={save}>
          ...
          <div className="form-group"><label>Role</label><input className="form-control" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} required /></div>
