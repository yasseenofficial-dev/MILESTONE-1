...
import { budgetApi, vendorApi } from '../../api/index.js';
import EventSubNav from '../../components/organizer/EventSubNav.jsx';
import { Loading, Modal } from '../../components/organizer/ui.jsx';
...
export default function BudgetPage() {
  ...
  const load = () => {
    ...
    Promise.all([budgetApi.getBudget(eventId), vendorApi.getCatalog()])
      .then(([bRes, vRes]) => { setData(bRes.data.data); setVendors(vRes.data.data); })
  ...
  return (
    <div>
        ...
        <table>
          ...
          <tbody>
            {categories.map((c) => {
              ...
              return <tr key={c.id}><td>{c.name}</td><td>EGP {c.plannedAmount.toLocaleString()}</td><td>EGP {actual.toLocaleString()}</td><td>EGP {(c.plannedAmount - actual).toLocaleString()}</td></tr>;
            })}
          </tbody>
        </table>
      </div>
          ...
          <tbody>
            {expenses.map((e) => (
              <tr key={e.id}>
                ...
                <td>{e.description}</td><td>{e.budgetCategory?.name || '—'}</td><td>{e.vendor?.name || '—'}</td>
      ...
      {modal === 'category' && (
        <Modal title="Add Budget Category" onClose={() => setModal(null)} onSubmit={async () => { await budgetApi.createCategory(eventId, { ...catForm, plannedAmount: parseFloat(catForm.plannedAmount) }); setModal(null); load(); }}>
          <div className="form-group"><label>Name</label><input className="form-control" value={catForm.name} onChange={(e) => setCatForm({ ...catForm, name: e.target.value })} required /></div>
          <div className="form-group"><label>Planned Amount (EGP)</label><input className="form-control" type="number" value={catForm.plannedAmount} onChange={(e) => setCatForm({ ...catForm, plannedAmount: e.target.value })} required /></div>
        </Modal>
      )}
      {modal === 'expense' && (
        <Modal title="Record Expense" onClose={() => setModal(null)} onSubmit={async () => { await budgetApi.createExpense(eventId, { ...expForm, amount: parseFloat(expForm.amount), budgetCategoryId: expForm.budgetCategoryId || null, vendorId: expForm.vendorId || null }); setModal(null); load(); }}>
          <div className="form-group"><label>Description</label><input className="form-control" value={expForm.description} onChange={(e) => setExpForm({ ...expForm, description: e.target.value })} required /></div>
          <div className="form-group"><label>Amount (EGP)</label><input className="form-control" type="number" value={expForm.amount} onChange={(e) => setExpForm({ ...expForm, amount: e.target.value })} required /></div>
